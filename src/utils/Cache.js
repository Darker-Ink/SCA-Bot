...
class Cache {
     ...
     * @returns {Promise<this>} The Redis Client
     */
    connect() {
            ...
            this._redis.on('ready', () => {
                ...
                this._pingInterval = setInterval(() => {
                    if (Math.floor(Date.now() - this._lastPing) > 15000 && this._lastPing) {
                        console.error(`Redis Failed to respond to an ping, Exiting... (Last Ping: ${(new Date(this._lastPing)).toLocaleString()})`);
                        process.exit();
    ...
    //                 for (const k of x) {
    //                     this._redis.get(k).then((y) => {
    //                         parsedData.push(JSON.parse(y));
    //                     });
    //                 }

    //                 while (parsedData.length != x.length) {
    //                     console.log(`The length of the parsed data is ${parsedData.length} and the length of the keys is ${x.length}`);
    //                 }
